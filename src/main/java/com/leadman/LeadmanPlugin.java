...
@PluginDescriptor(
	...
	tags = {"ironman", "bronzeman", "challenge", "unlock", "restriction"}
)
public class LeadmanPlugin extends Plugin
{
	private static final Set<String> USE_OPS = new HashSet<>(Arrays.asList(
		"eat", "drink", "wear", "wield", "equip"
	));
	...
	private void announceLevelUnlocks(Skill skill, int level, List<ItemRule> newly)
	{
		...
		if (newly.size() > config.popupBatchThreshold())
		{
			...
			chat(heading + " unlocked " + newly.size() + " items. See the Leadman panel for the list.");
	...
	public void onMenuEntryAdded(MenuEntryAdded event)
	{
		...
		if (option.startsWith("cast") || option.startsWith("autocast"))
		{
			if (!target.isEmpty() && !unlockService.canCast(target))
			{
				removeEntry(event);
				explainOnce("cast:" + target, "You cannot craft the runes for " + target
					+ " yet. Missing " + unlockService.missingRuneFor(target) + ".");
			}
			return;
		}

		boolean isUseOp = USE_OPS.contains(option);
		boolean isGeOp = geOpen && GE_OPS.contains(option);
		boolean maybeActivate = !isUseOp && !isGeOp && !INERT_OPS.contains(option);

		if (!isUseOp && !isGeOp && !maybeActivate)
		{
			return;
		}
		...
		if (isGeOp && config.blockGrandExchange() && !unlockService.canTradeKey(key))
		{
			removeEntry(event);
			explainOnce("trade:" + key, "Not tradeable yet: " + unlockService.displayFor(itemId)
				+ " needs " + unlockService.lockReason(key) + ", or obtain one.");
			return;
		}

		// Teleport options are named after their destination -- "Edgeville", "Duel Arena"
		// -- so there is no fixed list to match on. Anything that is not a known inert op,
		// on an item that carries a charge, is treated as spending that charge.
		if (maybeActivate && unlockService.isActivatable(key) && !unlockService.canActivateKey(key))
		{
			removeEntry(event);
	...
	{
		if (!profileLoaded || !config.blockGrandExchange())
		{
			return;
		}
	...
	private void chat(String message)
	{
		...
		client.addChatMessage(ChatMessageType.GAMEMESSAGE, "", "[Leadman] " + message, null);
	}
	...
	/** Drawn rather than shipped as a PNG, so the plugin carries no binary assets. */
	private static BufferedImage buildIcon()
	{
		...
		g.setColor(new Color(103, 113, 126));
		g.fillRoundRect(1, 6, 14, 9, 3, 3);

		g.setColor(new Color(226, 174, 74));
		g.setStroke(new BasicStroke(2f));
		g.drawArc(4, 2, 8, 9, 0, 180);
